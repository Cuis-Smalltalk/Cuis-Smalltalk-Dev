...
Smalltalk systems store source code in two files, the Sources file (named, for example 'Cuis6.3.sources') and the Changes file (for example, 'uis6.3-6202.changes'). The Sources file is not modified after creation, and new and modified code is appended to the Changes file. Both these files are used (for instance) by the Smalltalk Browser, when exploring the source code for the system.
