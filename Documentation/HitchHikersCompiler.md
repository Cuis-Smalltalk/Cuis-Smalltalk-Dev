...
push temp N (or push local N in VW) - push an object in temporary N of the current context. As mentioned before, arguments are temps too. For example, in a method of 2 arguments temps 0 and 1 are the message arguments, temps 2 and on are the temporary variables declared in the method.

push literal N - push an object at index N in the current method's literal frame (we will discuss literal frames when the time is right).
